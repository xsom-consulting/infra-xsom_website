...
One versioned identity for xSOM Consulting and xSOM AI Guard. Canonical source: `xsom-consulting/infra-xsom_website/design-system`. The console vendors this exact directory at `frontend/design-system`; it does not maintain a second theme.
...
The console serves package assets from `/signal-media/`; copy `frontend/design-system/assets/*` into `frontend/public/signal-media/` after syncing. The original local WOFF2 files are retained byte-for-byte. Font roles: Saira display, Inter body, JetBrains Mono labels/data. The original three-arrow SVG geometry is preserved in `assets/mark-source.svg`; `build.mjs` deterministically recolors it copper/bronze/neutral into `assets/mark.svg`.
...
Dark by default; light is independently specified. `xsom-signal-preferences` stores theme and reduced-motion choices locally. OS reduced motion always wins. A `signal-preference` window event lets host animations stop immediately. Sound is off each visit and only enabled by a gesture; no file/network/audio request is needed. `signalFeedback(verdict)` may be called only after an actual successful response for a live action.
