#!/usr/bin/env node
/** Scoped hero regression: real 736 CSS-pixel window, bilingual links and original logo. */
      ...
      for (const width of [1440, 736, 768, 390]) {
        ...
        for (const file of ['index.html', 'en/index.html']) {
          ...
          if (width >= 700) {
            ...
            assert.ok(result.logo.bottom < 734, 'The logo must be visible in the original user window');
          } else {
            assert.ok(result.logo.top > result.heading.bottom, 'Phone layout places logo after the headline');
            assert.ok(result.logo.bottom < 734);
          }
