...
for(const lang of ['fr','en']){
  await page.goto(`http://localhost:4173/design-system/og.html?lang=${lang}`);await page.evaluate(()=>document.fonts.ready);
...
for(const [size,name] of [[32,'favicon-32.png'],[180,'apple-touch-icon.png'],[512,'logo-512.png']]){
  ...
  await page.setContent(`<html><head><style>html,body{margin:0;width:100%;height:100%;background:transparent}img{display:block;width:100%;height:100%}</style></head><body><img src="http://localhost:4173/design-system/assets/mark.svg" alt=""></body></html>`);
